...
def _lines(data: dict) -> Iterable[tuple[str, list[str]]]:
    ...
    yield labels["experience"], experience
    education = []
    ...
    yield labels["education"], education
    ...
    for title, lines in _lines(data):
        ...
        if clean:
            sections.append(
                ...
                + "".join(f"<p>{line}</p>" for line in clean)
                + "</section>"
            )
...
<head>
<meta charset="utf-8" />
<title>{html.escape(str(data.get("fullName", "")))}</title>
<style>
  body {{ font-family: Arial, Tahoma, sans-serif; margin: 32px; color: #0f172a; }}
  .photo {{ width: 96px; height: 96px; border-radius: 50%; object-fit: cover; }}
  h1 {{ margin: 8px 0 2px; }}
  .contact {{ color: #475569; margin-bottom: 16px; }}
  h2 {{ color: #1e745a; border-bottom: 2px solid #e2e8f0; padding-bottom: 4px; }}
</style>
</head>
<body>
{photo_html}
<h1>{html.escape(str(data.get("fullName", "")))}</h1>
<p>{html.escape(str(data.get("headline", "")))}</p>
...
def export_docx(data: dict) -> bytes:
    ...
    if photo:
        p = doc.add_paragraph()
        p.alignment = align
        p.add_run().add_picture(io.BytesIO(photo), width=Inches(1.05))
    title = doc.add_heading(str(data.get("fullName", "")), 0)
...
def export_pdf(data: dict) -> bytes:
    ...
    for path in [
        ...
        try:
            pdfmetrics.registerFont(TTFont("CvUnicode", path))
            font = "CvUnicode"
            break
        except Exception:
            continue
    ...
    body = ParagraphStyle(
        "CvBody", parent=styles["BodyText"], fontName=font, fontSize=10,
        leading=15, alignment=TA_RIGHT if is_arabic else TA_LEFT,
    )
    heading = ParagraphStyle(
        "CvHeading", parent=body, fontSize=14, leading=20,
        textColor="#1e745a", spaceBefore=8,
    )
    title = ParagraphStyle("CvTitle", parent=body, fontSize=22, leading=28)

    def pdf_text(value: object) -> str:
        raw = str(value or "")
        # Ne reshaper QUE les lignes contenant de l'arabe (les lignes
        # latines restent lisibles par les ATS et les extracteurs).
        if is_arabic and AR_RE.search(raw):
    ...
    photo = _photo_bytes(data)
    if photo:
        story.extend([PdfImage(io.BytesIO(photo), width=26 * mm, height=26 * mm), Spacer(1, 5)])
    story.append(Paragraph(pdf_text(data.get("fullName", "")), title))

    # Coordonnées séparées : arabe (visuel) / latin (lisible machines)
    ...
    if contact_rtl:
        story.append(Paragraph(pdf_text(contact_rtl), body))
    if contact_ltr:
        story.append(Paragraph(html.escape(contact_ltr).replace("\n", " "), body))
    if contact_rtl or contact_ltr:
