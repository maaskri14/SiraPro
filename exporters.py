...
LABELS = {
    "ar": {"summary": "الملخص المهني", "experience": "الخبرة المهنية", "education": "التعليم", "skills": "المهارات", "languages": "اللغات", "extras": "معلوفات إضافية"},
...
def export_html(data: dict) -> bytes:
    ...
    for title, lines in _lines(data):
        ...
        if clean:
            html_sec = f'<section><h2>{html.escape(title)}</h2>'
            for l in clean:
                html_sec += f"<p>{l}</p>"
            html_sec += "</section>"
...
<head><meta charset="utf-8" /><title>{html.escape(str(data.get("fullName", "")))}</title>
<style>
